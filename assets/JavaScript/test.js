...
function createButtons(array)
{
  ...
  for (i=0;i<array.length;i++)
  {
    ...
    $("header").append($(button).text(array[i]).on("click", function(){
      buttonClicked(this);
      console.log(this);
    }));
  }
...
function newButton(query){
  ...
  $("header").append($(button).text(query).on("click", function(){
    buttonClicked(this);
  }));
}
...
{
  $("body").append($("<form class='searchForm'>"));

  $(".searchForm").append($("<label for='custom'>")).append($("<input type='text' class='form-control' id='custom'>")).append($("<input class='btn' id='searchBtn' type='submit' value='Search'>"));
...
function gifSearch(searchTerm)
{
  ...
  returnedObj.done(function(data) {
    ...
    if (data.data.length === 0)
    {
      ...
      $(".container").append($("<p id='noResults'>"));
      $("#noResults").text("Sorry, no results.");  
    }
    ...
    else 
    {
      ...
      for (var i=0;i<limit;i++)
      {
        ...
        $(".container").append($("<div class='gifHolder' id='"+data.data[i].id+"'>"));
